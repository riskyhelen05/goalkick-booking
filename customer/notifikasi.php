...
<div class="max-w-3xl mx-auto px-5 pb-16 mt-2">
  ...
  <div class="space-y-3">
    ...
    <div class="notif-card rounded-2xl border overflow-hidden <?=!$n['is_read']?'notif-unread':''?>"
      ...
      <div class="flex gap-4 p-4">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-start justify-between gap-2">
            <div class="font-bold text-sm text-white leading-tight"><?=htmlspecialchars($n['judul'])?></div>
            <div class="flex items-center gap-1.5 flex-shrink-0">
              <?php if(!$n['is_read']):?>
              <div class="w-2 h-2 rounded-full pulse-dot" style="background:<?=$c['text']?>"></div>
              <?php endif;?>
              <span class="text-xs text-gray-600 whitespace-nowrap"><?=timeAgo($n['sent_at'])?></span>
            </div>
          </div>
          <p class="text-sm mt-1 leading-relaxed" style="color:<?=$c['text']?>;opacity:.9"><?=htmlspecialchars($n['pesan'])?></p>

          <!-- Booking detail chip -->
          <div class="flex items-center gap-3 mt-2.5 flex-wrap">
            <span class="text-xs px-2.5 py-1 rounded-lg font-mono" style="background:rgba(0,0,0,.3);color:#9ca3af">
              <?=htmlspecialchars($n['kode_booking'])?>
            </span>
            <span class="text-xs text-gray-500">
              🏟 <?=htmlspecialchars($n['nama_lapangan'])?>
            </span>
            <span class="text-xs text-gray-500">
              📅 <?=date('d M Y',strtotime($n['tanggal']))?> · <?=substr($n['jam_mulai'],0,5)?>–<?=substr($n['jam_selesai'],0,5)?>
            </span>
          </div>
        </div>
      </div>
      ...
      $showAction = false;
      $actionBtn  = '';
      if(strpos($n['judul'],'Segera Dimulai')!==false) {
          $showAction = true;
          $actionBtn  = '<a href="riwayat.php" class="no-underline text-xs px-3 py-1.5 rounded-lg font-semibold transition hover:opacity-80" style="background:'.$c['icon_bg'].';color:'.$c['text'].'">📋 Lihat Detail Booking</a>';
      } elseif(strpos($n['judul'],'Berhasil Dibuat')!==false || strpos($n['judul'],'Booking Berhasil')!==false) {
          $showAction = true;
          $actionBtn  = '<a href="riwayat.php" class="no-underline text-xs px-3 py-1.5 rounded-lg font-semibold transition hover:opacity-80" style="background:'.$c['icon_bg'].';color:'.$c['text'].'">📋 Lihat Riwayat</a>';
      } elseif(strpos($n['judul'],'Dikonfirmasi')!==false) {
          $showAction = true;
          $actionBtn  = '<a href="riwayat.php" class="no-underline text-xs px-3 py-1.5 rounded-lg font-semibold transition hover:opacity-80" style="background:'.$c['icon_bg'].';color:'.$c['text'].'">✅ Lihat Booking</a>';
      } elseif(strpos($n['judul'],'Dibatalkan')!==false || strpos($n['judul'],'Ditolak')!==false) {
          $showAction = true;
          $actionBtn  = '<a href="booking.php" class="no-underline text-xs px-3 py-1.5 rounded-lg font-semibold transition hover:opacity-80" style="background:'.$c['icon_bg'].';color:'.$c['text'].'">🔄 Booking Ulang</a>';
      }
      ?>
